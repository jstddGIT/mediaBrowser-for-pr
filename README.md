...
以下为 v1 之后的规划，详见 [`docs/roadmap.md`](docs/roadmap.md)。

### 已提出需求

- [ ] **A1 视频静态缩略图**（首帧，需 Node+ffmpeg 抽帧 + 缓存）
- [ ] **A2 视频悬停擦洗预览**（受编码支持限制）— 大
- [ ] **A3 素材拖入时间线**（HTML5 拖拽，需真机验证落点）
- [ ] **A4 Bin 与素材按名称排序**（中文 localeCompare）
- [ ] **A5 左右面板大小可拖拽调整** 
- [ ] **A6 面板美化 + 紧凑布局**（缩小/折叠头部等）
- [ ] **A7 一键展开所有 Bin**（需进度提示/上限保护）
- [ ] **A8 素材标签**：读取 PR 中素材标记并展示在缩略图下方 
- [ ] **A9a 右键菜单：在项目中显示**（reveal 到项目面板）
- [ ] **A9b 右键菜单：标记优质素材 / 评级**（需自建本地元数据存储）
